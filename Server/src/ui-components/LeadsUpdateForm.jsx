...
export default function LeadsUpdateForm(props) {
  const {
    id: idProp,
    leads: leadsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = leadsRecord
      ? { ...initialValues, ...leadsRecord }
      : initialValues;
    setF_name(cleanValues.f_name);
    setL_name(cleanValues.L_name);
    setMake(cleanValues.Make);
    setModel(cleanValues.Model);
    setReg_Number(cleanValues.reg_Number);
    setSa_price(cleanValues.Sa_price);
    setDeposit(cleanValues.deposit);
    setBa_finance(cleanValues.ba_finance);
    setT_amount(cleanValues.t_amount);
    setBlaoo_payment(cleanValues.Blaoo_payment);
    setM_payment(cleanValues.m_payment);
    setRate(cleanValues.rate);
    setSta_date(cleanValues.sta_date);
    setM_of_payment(cleanValues.m_of_payment);
    setFigure(cleanValues.figure);
    setAgeement(cleanValues.ageement);
    setBebate(cleanValues.bebate);
    setValueation(cleanValues.valueation);
    setEquity(cleanValues.equity);
    setMilege_start(cleanValues.Milege_start);
    setMilege_now(cleanValues.Milege_now);
    setPhone(cleanValues.phone);
    setEmail(cleanValues.email);
    setP_code(cleanValues.p_code);
    setType(cleanValues.type);
    setStatus(cleanValues.status);
    setErrors({});
  };
  const [leadsRecord, setLeadsRecord] = React.useState(leadsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Leads, idProp)
        : leadsModelProp;
      setLeadsRecord(record);
    };
    queryData();
  }, [idProp, leadsModelProp]);
  React.useEffect(resetStateValues, [leadsRecord]);
  const validations = {
    f_name: [],
    L_name: [],
    Make: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Leads.copyOf(leadsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "LeadsUpdateForm")}
      {...rest}
    >
      <TextField
        label="F name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || leadsModelProp)}
